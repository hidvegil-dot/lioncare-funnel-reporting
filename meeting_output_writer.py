...
class MeetingOutputWriter:
    def __init__(self, drive: GoogleDriveClient, root_folder_name: str) -> None:
        ...
        self.meetings_folder_id = drive.ensure_folder_path(
            ["meeting_ai", MEETINGS_FOLDER_NAME],
    ...
    def write_outputs(self, *, transcript: dict[str, Any], analysis: dict[str, Any]) -> MeetingOutputLinks:
        ...
        meeting_folder_id = self.drive.ensure_folder(base, parent_id=self.meetings_folder_id)
        output_folder = f"{self.root_folder_name}/meeting_ai/{MEETINGS_FOLDER_NAME}/{base}"
        header = _output_header(analysis)

        crm_note_link = self._upload_markdown(
            folder_id=meeting_folder_id,
            filename="crm-note.md",
    ...
    def _upload_markdown(self, *, folder_id: str, filename: str, content: str) -> str:
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / filename
            path.write_text(content, encoding="utf-8")
            file_id = self.drive.upload_file(path, folder_id=folder_id, filename=filename)
